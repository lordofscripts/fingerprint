...
/* DO NOT REMOVE - Used by Taskfile - v2
//>>>BEGIN Versioner
package main
...
func main() {
    if len(os.Args) == 2 && strings.EqualFold(os.Args[1], "short") {
        fmt.Println(fingerprint.ModuleVersion.Short())
        } else if len(os.Args) == 2 && strings.EqualFold(os.Args[1], "version") {
                fmt.Println(fingerprint.ModuleVersion.Version())
    } else {
        fmt.Println(fingerprint.ModuleVersion)
    }
}
//>>>END Versioner
*/
